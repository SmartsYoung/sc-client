...
// addTokenToHeader carry auth token to header
func (c *Client) addTokenToHeader(token string) http.Header {
	header := http.Header{}
	header.Set(HeaderAuth, "Bearer "+token)
	return header
}
...
func (c *Client) GetAccount(accountName, token string) (*rbacframe.Account, error) {
	url := c.formatURL(fmt.Sprintf("%s/%s", AccountPath, accountName), nil, nil)
	resp, err := c.httpDo(http.MethodGet, url, c.addTokenToHeader(token), nil)
	if err != nil {
		return nil, err
	}
	if resp == nil {
	...
	if resp.StatusCode >= 200 && resp.StatusCode < 300 {
		var response rbacframe.Account
		err = json.Unmarshal(body, &response)
		if err != nil {
...
func (c *Client) GetAllUserAccounts(token string, opts ...CallOption) ([]*rbacframe.Account, error) {
	copts := &CallOptions{}
	for _, opt := range opts {
		opt(copts)
	}
	...
	if resp.StatusCode >= 200 && resp.StatusCode < 300 {
		var response rbacframe.AccountResponse
		err = json.Unmarshal(body, &response)
		if err != nil {
			return nil, NewJSONException(err, "parse the account info failed")
		}
...
func (c *Client) UnregisterAccount(userName, token string) (bool, error) {
	url := c.formatURL(fmt.Sprintf("%s/%s", AccountPath, userName), nil, nil)
	resp, err := c.httpDo(http.MethodDelete, url, c.addTokenToHeader(token), nil)
	if err != nil {
		return false, err
	}
	if resp == nil {
...
func (c *Client) GetRole(roleName, token string) (*rbacframe.Role, error) {
...
func (c *Client) GetAllRoles(token string, opts ...CallOption) ([]*rbacframe.Role, error) {
	copts := &CallOptions{}
	for _, opt := range opts {
		opt(copts)
	}
	...
	if resp.StatusCode >= 200 && resp.StatusCode < 300 {
		var response rbacframe.RoleResponse
		err = json.Unmarshal(body, &response)
		if err != nil {
			return nil, NewJSONException(err, string(body))
		}
...
func (c *Client) UpdateRole(role *rbacframe.Role, token string) error {
	if role == nil {
		return errors.New("invalid request role parameter")
	}
	...
	resp, err := c.httpDo(http.MethodPut, registerURL, c.addTokenToHeader(token), body)
	if err != nil {
		return err
	}
	if resp == nil {
...
func (c *Client) UnregisterRole(roleName, token string) (bool, error) {
	url := c.formatURL(fmt.Sprintf("%s/%s", RolePath, roleName), nil, nil)
	resp, err := c.httpDo(http.MethodDelete, url, c.addTokenToHeader(token), nil)
	if err != nil {
		return false, err
